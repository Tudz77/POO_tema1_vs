#include "Matrice_Complexa1.h"

Matrice_Complexa::Matrice_Complexa(double x, int n, int m)
{
    this->n = n;
    this->m = m;
    for (int i = 0; i < n; i++)
...
Matrice_Complexa::Matrice_Complexa(const Matrice_Complexa& other)
{
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            matrice[m * i + j] = other.matrice[m * i + j];
}
Matrice_Complexa::~Matrice_Complexa()
{
    n = 0;
...
double Matrice_Complexa::getpos(int i, int j)       //GET M[I][J]
{
    ...
    if (m1.get_m() == m2.get_m() && m1.get_n() == m2.get_n())
    {
        ...
        for (i = 0; i < result.get_n(); i++)
            for (j = 0; j < result.get_m(); j++)
                result.matrice[result.m * i + j] = m1.matrice[result.m * i + j] + m2.matrice[result.m * i + j];
